...
## 补充：You Don't Know JS 的经典"五规则"速记

Kyle Simpson 在《You Don't Know JS: this & Object Prototypes》里给出的四条判断规则 + 箭头函数例外，是面试里最常被问到的速记版本：

1. **默认绑定（Default Binding）**：函数独立调用，不依附任何对象——非严格模式 this 指向全局对象，严格模式为 `undefined`。
2. **隐式绑定（Implicit Binding）**：函数作为对象的方法被调用（`obj.fn()`），this 指向调用它的那个对象。
3. **显式绑定（Explicit Binding）**：用 `call`/`apply`/`bind` 强制指定 this；其中 `bind` 属于"硬绑定（hard binding）"，绑定后无法再被 call/apply 覆盖。
4. **new 绑定（new Binding）**：用 `new` 调用构造函数，this 指向新创建的实例。
5. **箭头函数**（书中作为例外单独讨论，不算第五条独立规则）：没有自己的 this，词法继承外层作用域，不参与前四条规则的判断和优先级排序。

判断顺序（书中给出的口诀）：**先看 new → 再看显式绑定（call/apply/bind）→ 再看隐式绑定（调用处的对象）→ 最后默认绑定**，四条从高到低依次判断，符合哪条就按哪条来；箭头函数直接跳过这整套判断流程。

这套速记覆盖了下面"函数上下文"里最常考的部分，但没覆盖 this 替换、类上下文、全局上下文这几块——面试快速抢答够用，系统理解还得看完整的 MDN 三分法。
